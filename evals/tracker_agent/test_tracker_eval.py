...
def write_result(action: str, passed: bool, latency_ms: int):
    EVAL_FILE.parent.mkdir(parents=True, exist_ok=True)
    row = {"action": action, "passed": passed, "latency_ms": latency_ms}
...
def test_tracker_eval_streak_shape():
    with patch("agents.tracker_agent.psycopg2.connect", side_effect=Exception("no db")):
        ...
        write_result("get_streak", passed, latency_ms)
        assert passed
...
def test_tracker_eval_log_shape():
    with patch("agents.tracker_agent.psycopg2.connect", side_effect=Exception("no db")):
        ...
        write_result("log_solve", passed, latency_ms)
        assert passed
...
def test_tracker_eval_topic_shape():
    with patch("agents.tracker_agent.psycopg2.connect", side_effect=Exception("no db")):
        ...
        write_result("get_topic_progress", passed, latency_ms)
        assert passed
